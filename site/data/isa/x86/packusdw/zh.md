...
使用无符号饱和到句柄溢出条件,将第一个和第二个 源操作数 中已包装的已签名双字整数转换为已包装的未签名单字整数. 如果签名的双字值超出一个未签名单字的范围(即大于FFFFH或小于0000H),则FFFFH或0000H的饱和无签名单字整数分别存储在目的地.

EVEX 编码版本 : 第一源操作数是一个ZMM/YMM/XMM登记册. 第二源操作数是一个ZMM/YMM/XMM的登记器,一个512/256/128位的内存位置,或者从32位的内存位置广播的512/256/128位矢量. 目标操作数是一个ZMM登记册,在写掩码 k1下有条件更新.
...
128位遗产 SSE 版本 : 第一源操作数是一个XMM登记册. 第二个操作数可以是XMM寄存器或128位内存位置. 目的地与第一个来源XMM注册点并不不同,对应目的地注册点目的地的上位位(MAXVL-1:128)没有修改.
...
Non-EVEX-encoded discription,参见表2-21,"Type 4类例外条件". EVEX-encoded discription,参见表2-52,"Type E4NF类例外条件".
