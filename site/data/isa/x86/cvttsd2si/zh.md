---
summary: 以 Truncation 标量 双精度浮点 转换为已签名值
---

## 说明

将源操作数(第二个操作数)中的双精度浮点值转换为目标操作数(第一个操作数)中的签名双字整数(或者如果操作数大小是64位). 源操作数可以是XMM的寄存器,也可以是64位的内存位置. 目标操作数是一个通用注册. 当源操作数是一个XMM的寄存器时,双精度浮点的值就包含在寄存器的低四字中.
...
软件应确保VCVTTSD2SI的编码与VEX.L=0. 用 VEX.L = 1 编码 VCVTTSD2SI 可能会在不同处理器代代遇到不可预测的行为.
