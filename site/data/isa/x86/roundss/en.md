...
Invalid (signaled only if SRC = SNaN). Precision (signaled only if imm[3] = `0; if imm[3] = `1, then the Precision Mask in the MXCSR is ignored and precision exception is not signaled.) Note that Denormal is not signaled by ROUNDSS.
