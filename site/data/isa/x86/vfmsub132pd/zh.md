...
使用三个 源操作数 在 打包双精度浮点值 上执行一组 SIMD 乘积-负积计算,并在 目标操作数 中写入乘积-负积结果. 目标操作数亦为第一源操作数. 第二个操作数必须是SIMD的登记册. 第三个源操作数可以是SIMD寄存器或内存位置.

VFMSUB132PD : (英语). 将两个,四个或八个打包双精度浮点值从第一源操作数乘以两个,第三个源操作数中的四个或八个打包双精度浮点值乘以两个. 从无限精度中间结果中减去第二源操作数中的2,4或8个打包双精度浮点值,进行四舍五入,并将所产生的2,4或8个打包双精度浮点值存储到目标操作数(第一源操作数).

VFMSUB213PD : (英语). 将2,4或8个打包双精度浮点值从第二源操作数乘以2,4或8个打包双精度浮点值在第一源操作数. 从无限精度中间结果中减去第三源操作数中两个,四个或八个包装的双精度浮点值,进行四舍五入,并将所产生的两个,四个或八个打包双精度浮点值存储到目标操作数(第一源操作数).

VFMSUB231PD : (英语). 将2,4或8个打包双精度浮点值从第二个来源乘以2,4或8个打包双精度浮点值在第三个源操作数. 从无限精度中间结果中减去第一源操作数中的2,4或8个打包双精度浮点值,进行四舍五入,并将所产生的2,4或8个打包双精度浮点值存储到目标操作数(第一源操作数).
