---
summary: Regreso Máximo valores en coma flotante de precisión simple escalares
---

## Descripción

Compara el bajo valores en coma flotante de precisión simple en el primer operando de origen y el segundo operando de origen, y devuelve el valor máximo a la palabra doble baja del operando de destino.
...
Si sólo un valor es un NaN (SNaN o QNaN) para esta instrucción, el segundo operando de origen, ya sea un NaN o un valor en coma flotante válido, está escrito al resultado. Si en lugar de este comportamiento, se requiere que el NaN de operando de origen sea devuelto, la acción de MAXSS se puede emular utilizando una secuencia de instrucciones, como una comparación seguida por AND, ANDN y OR.
...
VEX.128 y EVEX versión codificada: El primer operando de origen es un registro xmm codificado por VEX.vvvv. Los bits (127:32) del destino de registro XMM se copian de los bits correspondientes en el primer operando de origen. Bits (MAXVL:128) del registro de destino se ponen a cero.
...
El software debe asegurar que VMAXSS esté codificado con VEX.L=0. Codificar VMAXSS con VEX.L=1 puede encontrar un comportamiento impredecible en diferentes generaciones de procesadores.
