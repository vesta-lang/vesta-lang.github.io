...
对目标操作数(第一个操作数)和源操作数(第二个操作数)中包装的四字进行签名的SIMD比较. 如果第一个(目的地)操作数中的数据元素大于第二个(来源)操作数中的相应元素,则目的地中的相应数据元素设定为所有1s;否则,设定为0s.
...
EVEX编码为VPCMPGTD/Q: 第一源操作数(第二个操作数)是一个ZMM/YMM/XMM登记册. 第二源操作数可以是ZMM/YMM/XMM的寄存器,512/256/128位内存位置或512/256/128位向量从64位内存位置广播. 目标操作数(第一个操作数)是一个根据写掩码 k2更新的面具寄存器.
...
Non-EVEX-encoded discription,参见表2-21,"第4类例外条件". EVEX-encoded VPCMPGTQ,参见表2-51,"第E4类例外条件".
