...
收集了由BASE APDR和带有SCALE比例尺的索引矢量V INDEX指向的8个单精度/双精度断层点内存位置。 结果被写入矢量寄存器。 元素通过VSIB指定(即索引寄存器是矢量寄存器,持有打包指数). 元素只有在相应的掩码位为一时才会被加载. 如果元素的掩码位没有设置,则目的地寄存器的相应元素保持不变. 整个口罩寄存器将被本指令设定为零,除非它触发例外.
