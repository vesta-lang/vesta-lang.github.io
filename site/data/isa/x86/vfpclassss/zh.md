---
summary: 测试 标量 浮点数32 类型
---

## 说明

FPCLASSSS 指令检查 源操作数 中值低的 单精度浮点 特殊类别,由 imm8 字节中的设置位指定. Imm8中的每个设置位指定了浮点值的类别,输入数据元素被分类对照. 输入值所有指定类别的分类结果合并排列,形成输入元素的最后布尔结果。 其结果被写入一个面具寄存器k2中的低位,根据写作ask k1. Bits MAQQKL-1:目的地1个被清除.
