...
VFNMSUB132PD : (英语). 将两个,四个或八个打包双精度浮点值从第一源操作数乘以两个,第三个源操作数中的四个或八个打包双精度浮点值乘以两个. 从否定的无限精度中间结果中减去第二源操作数中的2,4或8个打包双精度浮点值,进行四舍五入,并将所产生的2,4或8个打包双精度浮点值存储到目标操作数(第一源操作数).

VFNMSUB213PD : (英语). 将2,4或8个打包双精度浮点值从第二源操作数乘以2,4或8个打包双精度浮点值在第一源操作数. 从否定的无限精度中间结果中减去第三源操作数中的2,4或8个打包双精度浮点值,进行四舍五入,并将由此产生的2,4或8个打包双精度浮点值存储到目标操作数(第一源操作数).

VFNMSUB231PD : (英语). 将2,4或8个打包双精度浮点值从第二个来源乘以2,4或8个打包双精度浮点值在第三个源操作数. 从否定的无限精度中间结果中减去第一源操作数中两个,四个或八个包装的双精度浮点值,进行四舍五入,并将所产生的两个,四个或八个打包双精度浮点值存储到目标操作数(第一源操作数).
