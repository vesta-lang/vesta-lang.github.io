...
第二源操作数是这些指令的负载形式的内存地址. 目标操作数是这些指令的存储形式的内存地址. 其他的操作数既是XMM登记册(用于VEX.128版本),也是YMM登记册(用于VEX.256版本).
...
VMASKMOV不应用于访问映射的 I/O 和未刻录的内存,因为它的单个负载或存储的存取和订购是具体执行的.
